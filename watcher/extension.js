...
>>>>>>> 3f044c1 (Build system so Axios is installed)
    }
...
var { getUrl, setAddr, setPort, setProtocol } = /* @__PURE__ */ (function() {
...
function activate(context) {
  ...
  const login = vscode.commands.registerCommand("watcher.login", onLogin);
...
async function onCheckServerConnection() {
  await axios.post(getUrl("/ping")).then((response) => {
...
async function onLogin() {
  ...
  await axios.post("http://10.126.1.248:8083/login", params, configForm).then((response) => {
    axios.defaults.headers.common["Authorization"] = `Bearer ${response.data.access_token}`;
