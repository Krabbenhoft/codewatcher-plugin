...
var configForm = { headers: { "Content-Type": "application/x-www-form-urlencoded" } };
var configJson = { headers: { "Content-Type": "application/json" } };
var docAbsoluteFilePath;
var docRelativeFilePath;
var docLangId;
var activeLine;
var docText = "";
var idleTime = 0;
...
var unsavedLogs = false;
var currentSelection = "";
function activate(context) {
  console.log('Congratulations, your extension "watcher" is now active!');
  const login = vscode.commands.registerCommand("watcher.login", async () => {
    await onLogin();
  });
...
async function onLogin() {
  ...
  await axios.post(`http://${userIP}/login`, params, configForm).then((response) => {
...
async function onCheckServerConnection() {
  ...
  await axios.post(`http://${userIP}/login`, loginData, configForm).then((response) => {
    axios.defaults.headers.common["Authorization"] = `Bearer ${response.data.access_token}`;
...
async function handleNewActiveEditor(editor) {
  if (unsavedLogs) {
    addChangeObject("end", /* @__PURE__ */ new Date(), "", "");
    await onWriteLogs();
  }
  cleanUp();
  if (editor) {
...
async function runTimer() {
  isTimerRunning = true;
  idleTime = 0;
  addChangeObject("start", /* @__PURE__ */ new Date(), "", activeLine);
...
async function handleFocusChange(state) {
  if (isTimerRunning) {
    if (state.focused) {
      addChangeObject("focus", /* @__PURE__ */ new Date(), "", "");
    } else {
      addChangeObject("unfocus", /* @__PURE__ */ new Date(), "", "");
      await onWriteLogs();
    }
  }
}
  ...
  if (isTimerRunning) {
    ...
    for (const change of event.contentChanges) {
      ...
      if (textInserted.length > 3) {
        addChangeObject("insertion", /* @__PURE__ */ new Date(), textInserted, activeLine);
      } else if (textInserted.length < 1) {
        const deletedText = extractDeletedPortion(docText, event.document.getText());
        addChangeObject("deletion", /* @__PURE__ */ new Date(), deletedText, activeLine);
      }
    }
...
async function handlePasteAction() {
  ...
  const pastedText = extractInsertedPortion(beforeText, afterText);
  await addChangeObject("paste", /* @__PURE__ */ new Date(), pastedText, "");
}
async function handleCopyAction() {
  ...
  if (clipboardText && clipboardText.trim() !== "") {
    await addChangeObject("copy", /* @__PURE__ */ new Date(), "", clipboardText);
  }
}
...
function initializeDocumentVariables(document2) {
  cleanUp();
  docAbsoluteFilePath = document2.fileName;
  docRelativeFilePath = getRelativePath(docAbsoluteFilePath);
  docLangId = document2.languageId;
  docText = document2.getText();
  docActive = true;
  activeLine = document2.lineAt(vscode.window.activeTextEditor.selection.active.line).text;
}
function cleanUp() {
  idleTime = 0;
  isTimerRunning = false;
